        ...
        public static void Main(string[] args)
        {
            ...
            using (var input = File.Open(inputPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                foreach (var current in FileFormats.TEA.Mafia3.Setups)
                {
                    ...
                    FileFormats.TEA.Decrypt(decrypted, 0, 8, FileFormats.TEA.Mafia3.Keys, current.Sum, current.Delta, current.Rounds);
                ...
                using (var output = File.Open(outputPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
                {
                    ...
                    while (left > 0)
                    {
                        ...
                        FileFormats.TEA.Decrypt(data, 0, block, FileFormats.TEA.Mafia3.Keys, setup.Sum, setup.Delta, setup.Rounds);
